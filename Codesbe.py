...
with open('parameters.txt', 'r') as f:
    for line in f:
        # Loại bỏ khoảng trắng và chỉ xử lý dòng có dấu '='
        line = line.strip()
        if '=' in line:
            name, value = line.split('=')
            parameters.append(value.strip())
chi0 = float(parameters[0])
T2 = float(parameters[1])       # (fs) dephasing time T2
wt = float(parameters[2])        # (fs) width of laser pulse
N = int(parameters[3])         # chosen number of discrete energy in
dt = float(parameters[4])         # (fs) time step
tm = float(parameters[5])       # (fs) max. time
emax = float(parameters[6])     # (meV) max. energy
d0 = float(parameters[7])       # (meV) detuning energy
Er = float(parameters[8])       # (meV) Bound energy
h = float(parameters[9])      # (meV.fs) Planck constant (h-bar)
de = emax / N      
t0 = -3 * wt       
# ==========================================
# 2. TIỀN XỬ LÝ
...
def xuat_file_text(filename1,filename2,t, eps, fe, fh, p_abs, Nt, Pt):
    with open(filename1, "w") as f:
        ...
        for i in range(len(t)):
            for j in range(len(n_arr)):
                ...
                f.write(line)
            f.write("\n")
    print(f"Đã xuất dữ liệu vào file {filename1}")
    with open(filename2,"w") as f:
        f.write(f"chi0={chi0},wt={wt},d0={d0},T2={T2}\n")
        header = f"{'t(fs)':>15} {'N(t)':>15} {'P(t)':>15}"
        f.write(header+"\n")
        f.write("-" * 75 + "\n")
        for i in range(len(t)):
            t_val=t[i]
            N_t=Nt[i]
            P_t=Nt[i]
            line = f"{t_val:15f} {N_t:15.6f} {P_t:15.6f}\n"
            f.write(line)
            f.write("\n")
    print(f"Đã xuất dữ liệu chi tiết vào file '{filename2}' thành công!")
...
def Ve_do_thi_3D(tieude, t_arr, fe, fh, p_abs):
    ...
    surf = ax.plot_surface(E_mesh, T_mesh, fe, cmap='viridis')
    ax.set_ylabel('Thời gian t (fs)')
    ax.set_xlabel('Năng lượng (meV)')
    ...
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111, projection='3d')
    surf = ax.plot_surface(E_mesh, T_mesh, fh, cmap='plasma')
    ax.set_ylabel('Thời gian t (fs)')
    ax.set_xlabel('Năng lượng (meV)')
    ...
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111, projection='3d')
    surf = ax.plot_surface(E_mesh, T_mesh, p_abs, cmap='viridis')
    ax.set_ylabel('Thời gian t (fs)')
    ax.set_xlabel('Năng lượng (meV)')
...
xuat_file_text("3D.txt","2D.txt",t, eps, fe, fh, p_abs, Nt, Pt)
...
wt = 25.0
t, eps, fe, fh, p_abs, Nt, Pt = SoDE(chi0, wt, d0, T2)
xuat_file_text("3D-wt=50.txt","2D-wt=50.txt",t, eps, fe, fh, p_abs, Nt, Pt)
Ve_do_thi_3D(f"wt_{wt}", t, fe, fh, p_abs)
#3. Thay doi delta0
wt = 50.0
d0=20
t, eps, fe, fh, p_abs, Nt, Pt = SoDE(chi0, wt, d0, T2)
xuat_file_text(f"3D-d0={d0}.txt",f"2D-d0={d0}.txt",t, eps, fe, fh, p_abs, Nt, Pt)
Ve_do_thi_3D(f"d0_{d0}", t, fe, fh, p_abs)
# 4. Thay đổi chi0
d0=100
wt=50
chi0_vals = [0.1, 0.5, 1.0, 1.5, 2*3.14]
ket_qua = []
for c in chi0_vals:
     chi0 = c
     ...
     xuat_file_text(f"3D-chi0={c}.txt",f"2D-chi0={c}.txt",t_val, eps_val, fe_val, fh_val, p_val, Nt_val, Pt_val)
...
for v in T2_vals:
     ...
     xuat_file_text(f"3D-T2={v}.txt",f"2D-T2={v}.txt",t_val, eps_val, fe_val, fh_val, p_val, Nt_val, Pt_val)
